...
from qbank_mcq.core.migrate import TARGET_VERSION
from qbank_mcq.core.updates import (
    UpdateCheckError,
    VersionInfo,
    check_for_update,
    fetch_version_info,
...
def test_spec_excludes_unused_qt_modules() -> None:
    ...
    spec = (PACKAGING / "qbank-mcq.spec").read_text(encoding="utf-8")
...
def test_spec_keeps_matplotlib_for_the_charts() -> None:
    """図(設計書 §14-3)は matplotlib で描く。**exclude すると画面が落ちる。**

    可視化が入る前の spec は matplotlib と numpy を「開発・テスト用」として
    除外していた。画面を足したあとにそのままにすると、問題詳細を開いた瞬間に
    ImportError になる。exe を作らないと出ない不具合なのでここで見張る。
    """
    spec = (PACKAGING / "qbank-mcq.spec").read_text(encoding="utf-8")
    for needed in ("matplotlib", "numpy"):
        assert f'"{needed}",' not in spec
...
def test_spec_entry_point_keeps_the_package_context() -> None:
    ...
    assert "from qbank_mcq.app import main" in entry


def test_installer_is_per_user_and_keeps_the_data() -> None:
    """ユーザー単位・管理者権限なし。アンインストールでデータを消さない(設計書 §15)。"""
    iss = (PACKAGING / "qbank-mcq.iss").read_text(encoding="utf-8")
    assert "PrivilegesRequired=lowest" in iss
    assert r"DefaultDirName={localappdata}\Programs\{#AppName}" in iss

    # コメントを外して見る。**`{userappdata}` をどこにも書かない**のが
    # 「データに触らない」の担保で、[UninstallDelete] 自体を置かないのが一番強い。
    body = re.sub(r"(?m)^\s*;.*$", "", iss)
...
def test_release_workflow_runs_tests_before_building() -> None:
    ...
    assert workflow.index("pytest -q") < workflow.index("pyinstaller packaging/qbank-mcq.spec")
    # 実行時・GUI・配布の依存をまとめて固定したロックを使う(`pip install` で最新を拾わない)
