"""実物の集計 CSV(``ssdb`` 方言)の取り込み。

``testdata/item_stats_2026_02.csv`` は採点システムが実際に出力したファイル。
設計書 §10.2 が想定していた書式と食い違っていたので、その差をどう吸収しているかを
ここで固定する。実データなので**数値をハードコードして突き合わせる**
(実装計画 §0「実データをテスト資産にする」)。
