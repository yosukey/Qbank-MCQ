...
#: 分類できない解答(集計 CSV の ``その他`` 列)。設計書 §10.2-(3)。
#:
#: **受験者は必ずどれか 1 列に数えられる**ので、これを N から外すと受験者数と
#: 正答率がずれる。31 パターンのどれでもない区分として別のキーで持ち、度数合計には
#: 入れる。ただし選択肢を 1 つも特定できていないため、周辺マーク率にも
#: 指示個数違反率にも算入しない(設計書 §12)。
OTHER = "?"
...
def pattern_columns() -> tuple[str, ...]:
    """設計書 v15 形式の度数列見出し 32 個(31 パターン + 空白)。

    現行の §10.2 形式は ``無解答`` と ``その他`` を使う。列名の対応は
    ``io.csv_stats`` の別名表が持つ。
    """
